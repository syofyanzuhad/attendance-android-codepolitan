...
if [ -z "$JDK" -a -s "${XDG_CONFIG_HOME:-$HOME/.config}/Google/AndroidStudioGameTools/studio.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.AndroidStudioGameTools/config/studio.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
for VM_OPTIONS_FILE in \
    ...
    ${XDG_CONFIG_HOME:-$HOME/.config}/Google/AndroidStudioGameTools/studio$BITS.vmoptions \
    $IDE_HOME.vmoptions \
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=AndroidStudioGameTools \
  "-Djb.vmOptionsFile=$VM_OPTIONS_FILES" \
  ${IDE_PROPERTIES_PROPERTY} \
  -Didea.platform.prefix=AndroidStudio -Didea.jre.check=true -Didea.platform.prefix=AndroidStudioGameTools -Didea.load.plugins=false -Didea.initially.ask.config=force-not \
